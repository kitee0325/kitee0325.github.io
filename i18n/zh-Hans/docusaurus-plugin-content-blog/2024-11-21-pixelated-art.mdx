...
<img src={useBaseUrl('/img/Pixelate Demo.gif')} alt="pixelated-art" />
...
- ❎ 利用现有的像素 AI 工具，如 PixelLab。
  ...
  > <img src={useBaseUrl('/img/Pixellab Animate Example.gif')} />
...
<img src={useBaseUrl('/img/Run Cycles.gif')} />
...
<img src={useBaseUrl('/img/Sprite Run.gif')} />
...
<img src={useBaseUrl('/img/Sprite Run 20x.gif')} />
...
- 将模型上传到`Mixamo`，执行自动骨骼绑定，并选择和应用一个奔跑动画模型。
  <img src={useBaseUrl('/img/Mixamo Tifa Running.gif')} />
- 在 Blender 中加载带有奔跑动画的.fbx 模型，并从特定相机角度渲染顺序帧。
- 使用 SD 对顺序帧进行像素化。我添加了像素化处理，将其转换为真正的像素。
- 使用 FFmpeg 导出 GIF。
  <img src={useBaseUrl('/img/3DRender2D Pixel Tifa.gif')} />
...
- 在 blender 中使用[pixel art add-on](https://lucasroedel.gumroad.com/l/pixel_art)，而不是 ComfyUI 中的 pixelate 自定义节点。
  <img src={useBaseUrl('/img/Blender Pixel Art Addon.gif')} />
...
>
  <img
    src={useBaseUrl('/img/Dragon Girl Run.gif')}
    style={{ width: 'auto', height: 'auto', flex: 1 }}
  />
  <img
    src={useBaseUrl('/img/Dragon Girl Run 268.gif')}
